...
class challengersController {
    ...
    async viewGlobleContests_page(req, res, next) {
        try {
            ...
            res.render("contest/allGlobelContests", { sessiondata: req.session.data,fantasy_type });
    ...
    async addpricecard_page(req, res, next) {
        try {
            ...
            if (getdata.status == true) {
                res.render('contest/addPriceCard',{ sessiondata: req.session.data, data:getdata.challenger_Details,contentName:getdata.contest_Name,positionss:getdata.position,priceCardData:getdata.check_PriceCard,tAmount:getdata.totalAmountForPercentage,amount_type:getdata.amount_type})
    ...
    async addprizecard_page(req,res,next){
        try {
            ...
            if (getdata.status == true) {
                res.render('contest/addPrizeCard',{ sessiondata: req.session.data, data:getdata.challenger_Details,contentName:getdata.contest_Name,positionss:getdata.position,priceCardData:getdata.check_PriceCard,tAmount:getdata.totalAmountForPercentage})
            }else if(getdata.status == false){
                req.flash('error',getdata.message)
                res.redirect('/view-all-global-contests-challengers')
            }
    ...
    async createCustomContest(req,res,next){
        try{
            ...
            if(getlunchedMatches.status == true){
                ...
                res.render("contest/createCustomContest",{ sessiondata: req.session.data, listmatches:getlunchedMatches.data,matchkey:mkey,matchData:getlunchedMatches.matchData,dates:getlunchedMatches.dates,fantasy_type,objfind});
    ...
    async addEditmatchpricecard(req,res,next){
        try{
            ...
            if(getPriceData.status == true){
                res.render('contest/addEditPriceCard',{sessiondata: req.session.data,data:getPriceData.challengeData,contentName:getPriceData.contest_Name,positionss:getPriceData.position,priceCardData:getPriceData.check_PriceCard,tAmount:getPriceData.totalAmountForPercentage,amount_type:getPriceData.amount_type});
            }else{
